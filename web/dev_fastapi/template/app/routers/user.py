...
def create_user(user: User, id: int = Depends(GetIncCounterId('test').get_id)):
    print('#', id)
...
def upload_file(file: UploadFile = File(...),):
    file_name = upload_object(file)
    return {'file_name': file_name}


@router.get('/file/{file_name}')
def get_file(file_name: str = Path(...)):
    response = get_object_json(file_name)
    return response
